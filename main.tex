...
for an incomplete list. Usually characters live inside a cylinder that is
spinning.
...
From the view of the onlooker, Miller simply rotates around the ship at some angular velocity $\omega$ from a starting angle of $\phi$ from the vertical. Given that the Radius of the ship is $R$, Miller's position can be given by:
...
\[
\includegraphics[width=3in]{MillerAndWater.png}
\]
...
\[
\includegraphics[width=3in]{MillerWaterU.png}
\]
...
\[
\includegraphics[width=1.5in]{MillerAxes.png}   
\]

In order to view $\vec{u}$ from Miller's frame, we will have to rotate $\vec{u}$ by $-\theta(t)$. This is because in Miller's frame, the observer's world was initially rotated by $-\phi$ and then continued to rotate at $-\omega t$. By a given time $t$, Miller considers the world to have rotated by $-\phi - \omega t$, which just happens to be $-\theta (t)$. Thus, the path that Miller sees the ball take is given by:
\begin{equation}
\begin{split}
...
\[
\includegraphics[width=3in]{arclengthLabeled.png}
\]
...
And taking the arcCosine so that $\gamma$ is:
\begin{equation}
\gamma  = arcCos(\frac{\vec{m}\cdot \vec{b}}{\lvert \vec{m} \rvert \lvert \vec{b} \rvert}) 
 = arcCos (\frac{\vec{m}\cdot \vec{b}}{rR})
\end{equation}
...
Now that we've derived the equations of motion that a ball would take if dropped from the ship, we can compare their height as a function of time to the height of a ball if dropped on a planet where gravity is (approximately) constant. Our end goal is to determine what kind of situations give realistic gravity from Miller's point of view. The x components for each method are both non-zero as a function of time, which does not line up (pun intended) with a ball dropped on Earth. While this is important, we will not analyze the x component until the next section where we discuss projectile motion. 
