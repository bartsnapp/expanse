...
\DeclareMathOperator{\atan2}{atan_2}
...
\texttt{https://en.m.wikipedia.org/wiki/O'Neill\_cylinder}
...
Thus the equations of motion for a projectile (arrow) ((seems a bit quick!))
...
Let $\langle r_0, \theta_{0}\rangle$ be the initial polar coordinates of the projectile.
Let $w$ be the angular velocity of the ship in radians per second.
Let $R$ be the radius of the ship.
Let $\theta_t$ be the angle that the projectile was thrown with respect to the thrower and let $V_0$ be the velocity in which it was thrown with respect to the thrower. 
Assume $\theta = -\frac{\pi}{2}$ so that the thrower is in the ship underbelly. Then we find:
\begin{align*}
	\vec{r}_0 &= r_0\langle \cos(\theta_{0}),\sin(\theta_{0})\rangle = \langle0,-r_0\rangle\\
	\vec{v}_0 &= V_0\langle \cos(\theta_{t} + \frac{\pi}{2} + \theta_{0}),\sin(\theta{t} +\frac{\pi}{2} + \theta_0)\rangle + \dot{\vec{r}_0}\\
	&= V_0\langle \cos(\theta_{t}),\sin(\theta_t)\rangle + r_0\langle-w\sin(\theta_0),w\cos(\theta_0)\rangle\\
	&= V_0\langle \cos(\theta_{t}),\sin(\theta_t)\rangle + r_0w \langle1,0\rangle
\end{align*}

Now let $\alpha$ be the angle in radians between the thrower and the projectile. We subtract $wt$ since the thrower is constantly rotating at $w$.

\begin{align*}
  \vec{p} &= \vec{r}_0+\vec{v}_0 t\\
  \alpha &= -wt + \atan2( \vec{p}\cdot\langle1,0\rangle, \vec{p}\cdot\langle0,1\rangle)
\end{align*}
Thus the coordinates of the projectile are:
\[
\langle x,y\rangle = \langle R\alpha, R- \sqrt{\vec{p}\cdot \vec{p}}\rangle
\]
