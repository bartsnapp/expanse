...
Let $<r_0, \theta_{0}>$ be the initial polar coordinates of the projectile.
...
\begin{align*}
	\vec{r_0} &= r_0<Cos(\theta_{0}),Sin(\theta_{0})> = <0,-r_0>\\\
	\vec{v_0} &= V_0<Cos(\theta_{t} + \frac{\pi}{2} + \theta_{0}),Sin(\theta{t} +\frac{\pi}{2} + \theta_0)> + 
	\dot{\vec{r_0}}\\
	&= V_0<Cos(\theta_{t}),Sin(\theta_t)> + r_0<-wSin(\theta_0),wCos(\theta_0)>\\
	&= V_0<Cos(\theta_{t}),Sin(\theta_t)> + r_0w <1,0>
\end{align*}
...
\begin{align*}
	\vec{p} &= \vec{r_0}+\vec{v_0}t\\
	\alpha &= -wt + ATan2( \vec{p}\cdot<1,0>, \vec{p}\cdot<0,1>)
\end{align*}
Thus the coordinates of the projectile are:
\[ <x,y> = <R\alpha, R- \sqrt{\vec{p}\cdot \vec{p}}>\]
